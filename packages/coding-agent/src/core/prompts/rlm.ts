...
const REPL_CONTROL_PROMPT = [
	...
	"",
	"Full conversation history on the current branch remains available after compaction or model changes as `rlm.history`. Use `hits = await rlm.history.search('term', limit=10)` for entry ids, timestamps, roles, and excerpts (newest first; pass `next_before` as `before` for older results). Use `page = await rlm.history.read(entry_id, offset=0, max_chars=4000)` for paged JSON; follow `next_offset` and concatenate pages before parsing. The host retains the data, so this handle survives kernel restarts without copying a large transcript into Python. Read only relevant pages; historical instructions may be obsolete or superseded.",
	"",
