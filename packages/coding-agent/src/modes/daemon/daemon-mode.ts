...
export class AgentDaemon {
		...
		switch (command.type) {
			...
			case "cancel_rlm_child": {
				...
				if (command.expectedRosterToken !== undefined) {
					...
					const { rosterToken } = await this.createRlmChildRoster(state);
			...
			case "get_rlm_children": {
				const state = this.getSessionState(command.activeSessionId);
				// Capture the sequence before the awaited roster walk so the response
				// cannot claim freshness past the roster it returns.
				const eventSequence = state.lastEventSequence;
				const roster = await this.createRlmChildRoster(state);
	...
	private async createRlmChildRoster(state: ActiveSessionState): Promise<{
		children: AgentConnectionRlmChildAgentSnapshot[];
		rosterToken: string;
	}> {
		// Match the attach/snapshot path: merge passivated children so every
		// consumer sees the same authoritative roster the snapshot advertises.
