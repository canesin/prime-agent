	...
	private async connectWorker(worker: ResidentWorker, timeoutMs: number): Promise<DaemonWorkerClient> {
		...
		while (Date.now() < deadline) {
			...
			try {
				...
				lastError = error;
				client.close();
				if (isSupervisorRecoveryCancelled(error) || error instanceof DaemonWorkerAuthenticationError) {
					throw error;
				}
				await delay(25);
			}
	...
	private async recoverUncertainWorkerOperations(worker: ResidentWorker): Promise<void> {
		...
		if (orphanProcessJournalPath) {
			try {
				...
				if (!reapFailed) clearOrphanProcessJournal(orphanProcessJournalPath);
			} catch (error) {
