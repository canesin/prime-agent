	...
	render(width: number): string[] {
		...
		const metaLines = [
			...
			...(this.options.getModelId
				...
				: []),
			...extraMetadata.map(({ label, value }) => {
				const labelPrefix = `${label} `;
				const valueWidth = Math.max(1, metaWidth - visibleWidth(labelPrefix));
				const displayValue =
					label === "cwd" || label === "cwd (py)"
						? truncatePathMiddle(value, valueWidth)
						: truncateToWidth(value, valueWidth);
				return `${theme.fg("dim", labelPrefix)}${theme.fg("muted", displayValue)}`;
			}),
			...(cwd === undefined
				? []
				: [
